...
// logging in to the app with email and password
export async function loginEmailPassword(prevState, formData) {
    const supabase = await createClient();
    const email = formData.get('email');
    ...
    return { error: false, message: `Logged in as ${email}` };
}

// logging in to the app with Magic Link
export async function loginOTP(prevState, formData) {
    const supabase = await createClient()
    const email = formData.get('email')
...
export async function updatePassword(prevState, formData) {
    ...
    console.log('Update Password Attempt - Password:', password, 'Confirm:', confirmPassword); // Debug

    if (!password || !confirmPassword) {
        return { ...prevState, message: 'All fields are required', error: true };
    }
    if (password !== confirmPassword) {
        return { ...prevState, message: 'Passwords do not match', error: true };
    }

    // Validate session
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) {
        return { ...prevState, message: 'User session not found. Please log in.', error: true };
    }

    const { data, error } = await supabase.auth.updateUser({ password });
    console.log('Update User Response - Data:', data, 'Error:', error); // Debug

    if (error) {
        return { ...prevState, message: error.message, error: true };
    }

    return { ...prevState, message: 'Password updated successfully!', error: false };
}
...
export async function updateEmail(prevState, formData) {
    ...
    console.log('Update Email Attempt - New Email:', newEmail, 'Password:', password); // Debug

    if (!newEmail || !password) {
        return { ...prevState, message: 'New email and password are required', error: true };
    }
    ...
    const { data, error } = await supabase.auth.updateUser({ email: newEmail, password });
    console.log('Update Email Response - Data:', data, 'Error:', error); // Debug

    if (error) {
        return { ...prevState, message: error.message, error: true };
    }

    return { ...prevState, message: 'Email update request sent. Check your new email for confirmation.', error: false };
}
