...
export async function updateDefaultView(prevState, formData) {
    ...
    const { error } = await supabase.auth.updateUser({ data: { defaultView } });
...
export async function updateProfile(prevState, formData) {
    ...
    const supabase = await createClient();
    const newData = {};
    for (let key of ["fullName", "email", "password"]) {
...
export async function updatePreferences(prevState, formData) {
    ...
    const supabase = await createClient();
    const newData = {};
    for (let key of ["theme", "currency", "currencySymbol"]) {
        const value = formData.get(key);
        if (value) newData[key] = value;
    }
    const { error } = await supabase.auth.updateUser({ data: newData });
...
export async function updateDateTimeSettings(prevState, formData) {
    const validated = settingsSchema.safeParse({
        defaultDate: formData.get('defaultDate'),
        defaultTime: formData.get('defaultTime'),
    })
    ...
    const supabase = await createClient();
    const newData = {};
    for (let key of ["dateFormat", "timeFormat", "time24h"]) {
        const value = formData.get(key);
        if (value) newData[key] = value;
    }
    const { error } = await supabase.auth.updateUser({ data: newData });
