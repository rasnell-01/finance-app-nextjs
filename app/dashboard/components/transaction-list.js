...
export default function TransactionList({ range, initialTransactions, initialSettings }) {
    ...
    return (
        <div className="space-y-8">
            {Object.entries(grouped).map(([date, { transactions, amount }]) => (
                <div key={date}>
                    <TransactionSummaryItem date={date} amount={amount} initialSettings={initialSettings} />
                    <Separator />
                ...
                <div className="flex justify-center">
                    <Button variant="ghost" onClick={handleClick} disabled={loading}>
                        <div className="flex items-center justify-center min-h-[1.5em]">
                            {loading ? <LoaderPinwheel className="animate-spin" /> : <span>Load More</span>}
                        </div>
                    </Button>
                </div>
            )}
